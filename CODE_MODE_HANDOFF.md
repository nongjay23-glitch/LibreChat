...
- CW-1B.12 adds `/ask <topic>` as a dedicated requirement-question flow. It uses the existing Cowork planner endpoint and decision-card UI, forces `responseMode: "decision"`, and continues asking one high-impact question at a time without producing a plan card or Codex prompt unless the user later runs `/plan`. It also hardens planner JSON/quality handling with fenced/balanced JSON extraction plus safe fallback output when a selected model ignores strict JSON, returns empty retry/repair output, or still fails the quality gate after retry.
- CW-1B.12.1 tightens decision-question quality. Backend validation now rejects overloaded decision questions, fallback output uses a compact display topic instead of internal continuation prompts, and decision answers explicitly tell Cowork not to ask the same question again.
- CW-1B.12.2 makes fallback non-blocking. Fallback output is returned with warnings even if it misses the quality gate, so `/ask` and `/plan` do not dead-end in an error card when the selected model repeatedly fails the planner contract.
- CW-1B.12.3 adds language and repeat protection for decision questions. Cowork planner requests carry a `languageHint` and recent decision questions to avoid, backend validation rejects wrong-language or repeated questions, fallback rotates to a different safe question when possible, and Cowork decision labels are localized for the current Thai workflow.
- CW-1B.12.4 makes decision cards behave like a current question rather than permanent chat history. Answered decision cards are marked with the stored question/answer and hidden from the visible room so repeated `/ask` follow-ups do not push the chat down with stale question cards.
- CW-1B.12.5 fixes Thai mojibake in fallback planner output. Backend fallback plan/ask Thai strings were rewritten as valid UTF-8, and Cowork localStorage normalization now replaces legacy saved planner fields that contain broken encoding with readable recovery text.
- CW-1B.12.6 makes Cowork planner output more chat-first. Planner results render as assistant text with advanced details and Codex prompt hidden behind a disclosure, decision cards show one compact current question with concise choices, planner summaries animate like normal assistant replies, and follow-up planner payloads avoid leaking internal labels into user-facing questions.
